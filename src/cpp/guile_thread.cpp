...
//   };
  ...
  std::string s_path = ("\"" + QCoreApplication::applicationDirPath() +"/"+"./qmapper_std.lisp\"").toStdString();   
  const char *path = s_path.c_str();
  printf("Loading stdlib from %s\n", path);
  cl_object cl_path = c_string_to_object(path),
    stream = cl_open(1, cl_path);
  
  cl_load(1, stream);
  puts("qmapper's stdlib loaded!");
  cl_close(1, stream);
