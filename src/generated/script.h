...
class Script: public Propertierbase {
 ...
 public: virtual void setScript_type(scriptTypes val);
  ...
  virtual void set(flyweight<std::string> propertyname, std::string value) {
    ...
    const std::function<void(std::string)> error_rep = [&](std::string msg) {
      std::vector<std::string>& vec = field_error_map[ns_name.get()];
      printf("Pushing %s\n", msg.c_str());
      vec.push_back(msg);
      return;};
    if(propertyname == std::string("ns")  && doesntContainNs(value, error_rep)) { Ns_field = value; return; } }
