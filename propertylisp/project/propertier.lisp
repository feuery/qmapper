...
(defun fill-property-names (classname)
  (format t "~A() {~%" (symbol-name classname))
...
(defun source (forms)
  (let ((*print-case* :downcase)
	(classname (symbol-name (cadadr forms))))
    
    (format t "///////// CPP FILE STARTS HERE ~%")
    (format t "// classname on ~A~%" classname)
    (dolist (x property-container)
      (let ((name (symbol-name (car x)))
	    (type (symbol-name (cdr x))))
		    ...
		    do (let* ((to-compile (pop-queue *compilation-queue*))
			 ...
			 (if (and (string= file-type "def")
			     ...
			     (let ((file-contents (read-file to-compile)))
			       (let ((headd (stdout-str (headers file-contents)))
				     (cpp (stdout-str (source file-contents))))
				 (let ((header-filename (str output-dir "/" (symbol-name (cadr *compiled-class*)) ".h"))
				       (cpp-filename (str output-dir "/" (symbol-name (cadr *compiled-class*)) ".cpp")))
