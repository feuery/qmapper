...
(defun handle-base-class (property-types)
  ...
  (let ((header-filename (str *output-dir* "/propertierbase.h"))
       ...
       (->> (remove-duplicates property-types :test #'equal)
	    (mapcar #'(lambda (type)
			(str
			 ...
			 (format nil "virtual ~a get(const char* propertyname, bool *success, ~A* type_helper);~%" type type))))
	    (apply #'str))
       f)		       
    ...
    (with-open-file (f cpp-filename :direction :output :if-exists :overwrite :if-does-not-exist :create)
      ...
      (write-string (->> (remove-duplicates property-types :test #'equal)
			 (mapcar #'(lambda (type)
				     (str
				      (format nil "~%~%void Propertierbase::set(const char* propertyname, ~A value)~% { }~%" type)
				      (format nil "~a Propertierbase::get(const char* propertyname, bool *success, ~A* type_helper) ~%{ *success = false; ~A a; return a; }~%" type type type))))
			 (apply #'str))
		    f))
